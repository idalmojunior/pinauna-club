...
- **`/evento/controle/`** — tela de controle de chegada, protegida pela mesma `ADMIN_API_KEY`. No dia da prova, escolha a prova (2km/500m/200m/Aquathlon) e toque no nome (com número) de cada atleta conforme ele chega na borda — a ordem dos toques vira a posição. A lista de chegadas já mostra, na hora, a posição geral e a posição na categoria de cada um, sem precisar abrir outra tela. Tem botão "Desfazer" pra corrigir toque errado e link pra lista completa de inscritos.
...
- `evento-chegada.js` (admin) — registra/desfaz a chegada de um atleta numa prova. A listagem já devolve a posição geral e a posição na categoria de cada um calculadas na hora, pra tela de controle mostrar tudo junto sem precisar abrir o ranking numa aba separada — durante a prova não dá tempo de ficar trocando de tela.
